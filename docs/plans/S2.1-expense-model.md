...
|----|-----------|----------|
| T1 formatCents | ✅ Pass | `money.test.ts` — 184257→"$1,842.57", 0→"$0.00", 5→"$0.05" |
| T2 integer-cents math, no float drift | ✅ Pass | `money.test.ts` sumCents (100×1¢=100, exact) |
| T3 paginate math + label | ✅ Pass | `pagination.test.ts` — full/middle/clamp/empty |
| T4 ExpenseTable render | ✅ Pass | `ExpenseTable.test.tsx` — amount `.tnum` + right-aligned |
| T5 empty-state ⌘K | ✅ Pass | `ExpensesEmptyState.test.tsx` |
| T6 amountCents Int round-trip + onDelete:Restrict guard | ✅ Pass | `expense.int.test.ts` (local DB) |
| T7 seeded list renders date desc, 25/page | ✅ Pass | live: 23 seeded expenses render, date desc; screenshot below |

29 tests pass · lint clean · build green · typecheck clean. Live render (seeded `demo@billd.dev`): see `design-assets/docs/design/s2.1-expenses.png`.
